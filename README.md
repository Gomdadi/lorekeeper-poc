...
`lorekeeper` 패키지가 노출하는 공개 API. 인덱싱(쓰기)과 검색(읽기) 두 진입점으로 나뉜다.
...
| 구분 | 내용 |
|---|---|
| `chapter: int` | 회차 번호. `Event.chapter`·`Chapter.number`·`Chunk.chapter`의 근거 |
| `text: str` | 회차 원고 전체 |
| 반환 `dict` | `{chapter, labels(라벨별 누적 노드 수), rels(관계 타입별 누적 수), tokens{request,response,total}, summary(회차 요약)}` |

- **async 함수** — 이벤트 루프에서 await한다. 여러 회차 연속 시 `asyncio.run()`을 회차마다 부르지 말고 단일 루프에서 `await`로 이어 호출한다.
- **한 번 호출 = 한 회차** 누적 인덱싱(이전 회차 그래프 위에 upsert). **회차는 오름차순 순차 실행.**
- 검색 인덱스(벡터 `chunk_emb` · 풀텍스트 `chunk_text_ft` cjk)를 **자동 생성**한다.
...
```python
def build_retrievers()      -> dict[str, Retriever]   # 이름→인스턴스 (키 4종)
...
각 retriever의 `.search(...)` 시그니처와 반환:

| 도구 키 | `.search(...)` 인자 | 반환 |
|---|---|---|
| `vector_cypher` | `query_text: str`, `top_k: int = 5` | `RetrieverResult` |
| `hybrid_cypher` | `query_text: str`, `top_k: int = 5` | `RetrieverResult` |
| `entity_state_history` | `entity_name: str`, `up_to_chapter: int \| None = None` | `RetrieverResult` |
| `text2cypher` | `query_text: str` | `RetrieverResult` |

**반환 타입**:

```python
RetrieverResult:
    items: list[RetrieverResultItem]   # 결과 아이템
    metadata: dict                     # 검색 수준 메타(text2cypher는 metadata["cypher"]에 생성 쿼리)

RetrieverResultItem:
    content: str    # LLM 프롬프트에 그대로 넣는 자족 텍스트(원문 발췌 + 관련 그래프)
    metadata: dict  # 구조화 필드(chapter/chunk_index/score/nodes/relationships 등, 근거 추적용)
```
...
r = build_retrievers()
res = r["vector_cypher"].search(query_text="김독자가 겪은 사건", top_k=5)
context = "\n\n".join(item.content for item in res.items)   # 프롬프트에 바로 주입

r["entity_state_history"].search(entity_name="김독자", up_to_chapter=5)
r["text2cypher"].search(query_text="3~5화에 일어난 사건")   # res.metadata["cypher"]에 생성 Cypher

tools = build_retrieval_tools()   # 에이전트(LangGraph 등)에 배선할 Tool 리스트 (어댑터 필요, 아래 검색 계층 참고)
```

> 검색 인덱스는 `indexing()`이 자동 생성하므로 소비 쪽은 검색 전 별도 준비가 필요 없다. 세부 스펙은 `.claude/docs/querying-function-spec.md`·`indexing-function-spec.md` 참고.
