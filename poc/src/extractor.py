...
- state에는 그 사실을 그 자체로 읽히게 쓴다(예: '어깨를 칼날에 깊게 베임', '화산파에 정식 입문',
  '코인 6200 보유', '스물여덟 살'). 이 노드만 따로 읽혔을 때도 뜻이 통해야 한다 — 원문 인용은
  evidence가 담당하므로 원문 문장을 그대로 옮기지 말고 사실로 압축한다.
- 인물의 상태가 원문에 명시적으로 제시되면(서술이든 목록·표·공지 형태든) CharacterState로 만든다.
  원문이 값을 직접 확정해 준 것은 해석 여지가 없으므로 우선 추출 대상이다.
...
- 구조 완전성: 모든 description은 해당 노드·관계를 설명하는 부가 속성일 뿐이다. 노드/관계와 내용이
...
- evidence: 각 Event·CharacterState에, 그 사실의 근거가 되는 원문 문장을 그대로/가깝게 인용한다.
  해석·설명·요약을 덧붙이지 말고 원문에 실제로 있는 문장만 쓴다.
